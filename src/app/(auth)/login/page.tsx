...
import { Workflow, AlertCircle, ArrowRight, CheckCircle2 } from "lucide-react"
        ...
        <Card className="border-slate-200 dark:border-slate-800 shadow-sm bg-white dark:bg-slate-900">
          <form onSubmit={handleLogin}>
            <CardContent className="pt-6 space-y-4">
              {/* Security Best Practice: Inline Error Banner without disclosing specific field */}
              {error && (
                <div className="p-3.5 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900/50 rounded-lg flex items-start space-x-3 text-red-800 dark:text-red-300 text-xs">
                  <AlertCircle className="h-4 w-4 text-red-600 dark:text-red-400 shrink-0 mt-0.5" />
                  <div>
                    <span className="font-semibold">Invalid credentials</span>
                    <p className="text-red-600 dark:text-red-400 mt-0.5">Please check your email and password and try again.</p>
                  </div>
                </div>
              )}
              ...
              <Button
                type="button"
                variant="outline"
                className="w-full justify-center space-x-2 border-slate-300 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200"
              ...
              <div className="relative flex items-center justify-center my-3">
                ...
                <span className="relative bg-white dark:bg-slate-900 px-2 text-xs text-slate-400 dark:text-slate-500 uppercase tracking-wider">or email</span>
              </div>
