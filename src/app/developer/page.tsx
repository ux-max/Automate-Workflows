...
export default function DeveloperHubPage() {
  ...
  const getStatusBadge = (status: DeveloperAppStatus) => {
    switch (status) {
      ...
      case "private":
        return (
          <Badge variant="outline" className="text-[10px] uppercase font-bold text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
            Private (Dev)
          </Badge>
        )
      case "in_review":
        return (
          <Badge variant="outline" className="text-[10px] uppercase font-bold text-amber-700 dark:text-amber-300 border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/50">
            In Review
          </Badge>
        )
      case "changes_requested":
        return (
          <Badge variant="outline" className="text-[10px] uppercase font-bold text-red-700 dark:text-red-300 border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/50">
            Changes Requested
          </Badge>
        )
      case "public_beta":
      ...
      case "published":
        return (
          <Badge variant="outline" className="text-[10px] uppercase font-bold text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/50">
            Verified
          </Badge>
        )
      default:
      ...
      <div className="w-full bg-white dark:bg-slate-900 p-5 md:p-7 rounded-[22px] border border-slate-200/90 dark:border-slate-800 shadow-2xs space-y-6 relative">
          ...
          <div className="flex items-center space-x-3">
            <Link href="/developer/admin">
              <Button variant="outline" size="sm" className="space-x-1.5 text-xs font-semibold h-9">
                ...
                {stats.inReview > 0 && (
                  <span className="ml-1 px-1.5 py-0.2 rounded-full text-[10px] bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300 font-bold">
                    {stats.inReview}
                  </span>
                )}
              </Button>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card className="border-slate-200/90 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/40 shadow-2xs">
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Private / Dev Sandbox</p>
                <p className="text-2xl font-bold text-slate-900 dark:text-slate-100 mt-1">{stats.private}</p>
              </div>
          ...
          <Card className="border-slate-200/90 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/40 shadow-2xs">
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">In Review</p>
                <p className="text-2xl font-bold text-amber-600 dark:text-amber-400 mt-1">{stats.inReview}</p>
              </div>
          ...
          <Card className="border-slate-200/90 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/40 shadow-2xs">
            <CardContent className="p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400 mt-1">{stats.publicBeta}</p>
              </div>
        ...
        {filteredApps.length === 0 ? (
          <div className="p-12 text-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl">
            ...
            <h4 className="text-sm font-bold text-slate-700 dark:text-slate-300">No custom apps found</h4>
            ...
            <Button
              size="sm"
              onClick={() => setIsCreateDrawerOpen(true)}
              className="mt-4 bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs space-x-1.5 cursor-pointer"
            >
              <Plus className="h-4 w-4" />
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <CardTitle className="text-base font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2 truncate">
          ...
          <Card className="border border-slate-200/90 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xs rounded-xl overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 font-semibold uppercase text-[10px] tracking-wider">
                  <tr>
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="w-8 h-8 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 flex items-center justify-center shrink-0 overflow-hidden">
                            {app.logoIcon?.startsWith("data:") || app.logoIcon?.startsWith("http") ? (
                              ...
                              <div className="w-full h-full rounded-lg bg-blue-600 text-white font-bold flex items-center justify-center text-xs">
                                {app.name.charAt(0).toUpperCase()}
                              </div>
                            )}
                          </div>
